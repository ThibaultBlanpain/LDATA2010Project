...
def ReadFile():
    ...
    MajorData = fichier
    return
button = Button(fenetre, text="Import Data from *.csv", command=ReadFile).pack()
...
def ClearData():
    ...
    print(MajorData)
    return
button = Button(fenetre, text="Clear Imported Data from *.csv", command=ClearData).pack()


# bouton de sortie => ne fonctionne pas chez moi
bouton=Button(fenetre, text="Fermer", command=fenetre.quit).pack()
...
Button(text="Bar chart", command=callback2).pack()
